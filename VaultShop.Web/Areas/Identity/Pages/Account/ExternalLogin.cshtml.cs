...
namespace VaultShop.Web.Areas.Identity.Pages.Account
{
    ...
    public class ExternalLoginModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetCallbackAsync(string returnUrl = null, string remoteError = null)
        {
            ...
            if (remoteError != null)
            {
                ErrorMessage = $"{_localizer["ErrorExternalProvider"].Value} {remoteError}";
                return RedirectToPage("./Login", new { ReturnUrl = returnUrl });
            }
            var info = await _signInManager.GetExternalLoginInfoAsync();
            if (info == null)
            {
                ErrorMessage = _localizer["ErrorLoadExternalInfo"].Value;
                return RedirectToPage("./Login", new { ReturnUrl = returnUrl });
            }
        ...
        public async Task<IActionResult> OnPostConfirmationAsync(string returnUrl = null)
        {
            ...
            if (info == null)
            {
                ErrorMessage = _localizer["ErrorLoadExternalInfoConfirmation"].Value;
				return RedirectToPage("./Login", new { ReturnUrl = returnUrl });
            }
                ...
                if (result.Succeeded)
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        await _emailSender.SendEmailAsync(Input.Email, _localizer["ConfirmYourEmail"].Value,
                            $"{_localizer["ConfirmAccount"].Value} <a href='{HtmlEncoder.Default.Encode(callbackUrl)}'>{_localizer["ClickingHere"].Value}</a>.");
